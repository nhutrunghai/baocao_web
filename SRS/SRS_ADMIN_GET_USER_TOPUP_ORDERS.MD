...
## Chuc nang: Quan tri xem danh sach lenh nap vi cua nguoi dung
...
sequenceDiagram
    ...
    FE->>BE: GET /admin/users/:userId/wallet-topup-orders
...
**Ma chuc nang:** ADMIN-USER-TOPUP-ORDERS-01  
...
Chuc nang cho phep admin theo doi toan bo lich su tao lenh nap vi cua mot user. API hien tai duoc trien khai tai `GET /admin/users/:userId/wallet-topup-orders`.
...
* Query loc / phan trang theo validator `getAdminUserTopUpOrdersValidator`.

#### 3.2. Du lieu dau ra (Response Data)
* Danh sach top-up orders cua user cung trang thai, so tien va thoi gian tao.

#### 3.3. Du lieu luu tru / truy xuat
* Collection `wallet_topup_orders` de lay lich su don nap tien cua user.

### 4. Rang buoc ky thuat & bao mat (Technical Constraints)
* Chi admin moi xem duoc du lieu nay.
* Du lieu phai duoc loc dung theo userId muc tieu.

### 5. Truong hop ngoai le & xu ly loi (Edge Cases)
* **Truong hop:** User khong ton tai.  
  * **Xu ly:** Tra `404`.
* **Truong hop:** User chua co order nao.  
  * **Xu ly:** Tra danh sach rong.

### 6. Giao dien (UI/UX)
* Admin nen co bang lich su order voi bo loc trang thai.
* Nen cho phep drill-down sang chi tiet giao dich neu can.
